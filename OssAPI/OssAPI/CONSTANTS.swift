//
//  BackendCONSTANTS.swift
//  OssAPI
//
//  Created by Osama Al-Najjar on 5/13/16.
//  Copyright © 2016 Osama Al-Najjar. All rights reserved.
//

import Foundation

/// The struct CONSTANTS stores the global values used in the app.
struct CONSTANTS {
    
    /// Store the values related to the backend of the app.
    struct Backend {
        
        /// Define the api url server.
        static let API_URL = "http://beautyloopdummyapi.azurewebsites.net/api"//"http://private-2e13f-beautyloop.apiary-mock.com"
        
        /// Define the max timeout time for the api call.
        static let TIME_OUT: NSTimeInterval = 15

    }
    
    /// Store the values related to the frontend of the app.
    struct Frontend {
        
    }
    
}